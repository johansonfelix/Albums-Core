...
public class LogManager {
    ...
    public LogEntry findLogEntry(int logID){
        if(connection == null){
            return null;
        }
        else {
            ...
            ArrayList<LogEntry> entries = getLogEntriesFromResultSet(results);
            if(entries.size() != 1){
    ...
    private ArrayList<LogEntry> getLogEntriesFromResultSet(ResultSet results){
        ...
        try {
            while(results.next()){
                ...
                logEntry.setT(LogEntry.stringToTypeOfChange(results.getString(typeOfChangeColName)));
    ...
    public boolean insertLogEntry(LogEntry logEntry){
        if(connection == null){
            ...
            String query = "INSERT INTO LogEntries VALUES (" +
                    ...
                    LogEntry.typeOfChangeToString(logEntry.getT()) + "," +
                    logEntry.getISRC();
            query += ")";
            return true;
        }
    }
    public boolean updateLogEntry(int logID, LogEntry logEntry){
        String query = "UPDATE LogEntries SET ";
        query += timestampColName + "=" + logEntry.getTimestamp().toString() + ",";
        query += typeOfChangeColName + "=" + LogEntry.typeOfChangeToString(logEntry.getT()) + ",";
